...
Actually, the browser constructs a DOM incrementally and you can take advantage of this to speed up the rendering of your pages. In fact, this is something that the Google search pages do really, really well.
...
We can't really use a partial CSSOM tree, because then it could lead us to use the wrong styles when we render the page. The browser blocks page rendering, until it receives and processes all of the CSS. CSS is render blocking.
...
We need to combine the DOM and the CSSOM trees into the render tree. One of the most important properties of the render tree is that it only captures visible content.
...
This on load technique is a good one to have in your back pocket, but there's actually an even simpler strtegy that we can use. All we need is an extra attribute on the script tag.
...
`async` attribute has two important properties. First, it tells the browser that it does not have to block DOM construction, when it encounters the script tag. As a result, the browser dispatches the script request, and continues parsing the DOM. Second, the execution of the script also does not block on the CSS object model. So if the script is avaiable before the CSSOM is ready, it can still be executed right there and then. 
...
One of the best things about doing the critical rendering path analysis is you can get a pretty good estimate for how long the page will take to render. Once have the critical rendering path diagram I don't even have to profile my page. Well, I do need to profile my page for other bottlenecks, but what i mean is that i can see all the dependencies in my critical rendering path diagram and get a good idea of how long my page is going to take to render.
